...
void printCV8UImage(const cv::Mat& image) {
    if (image.depth() != 0)throw std::exception();
    int count255 = 0;
    int countOther = 0;
    ...
    std::cout << "255 count: " << count255 << std::endl;
    std::cout << "0 count: " << countOther << std::endl;
}
...
bool fitPoints(std::vector<cv::Point>& pts, Line& line) {
    ...
    if (nPoints < 2) {
        ...
        std::cout << "fitPoints: less than 2 points" << std::endl;
        return false;
    }
